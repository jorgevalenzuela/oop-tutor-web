...
type ViewState =
  | { mode: 'idle' }
  | { mode: 'in_exam'; exam: ExamInstance; questions: ExamQuestionMeta[]; currentIndex: number; currentQuestion: ExamQuestion }
  | { mode: 'results'; summary: ExamSummary; mastery: ConceptMastery[] }
...
export default function ExamPage() {
  ...
  async function handleStartExam() {
    ...
    try {
      ...
      setView({ mode: 'in_exam', exam, questions, currentIndex: 0, currentQuestion: firstQuestion })
    } catch (err) {
  ...
  async function handleSubmitAnswer(answer: string, timeSeconds: number): Promise<AnswerResult> {
    ...
    return assessmentApi.submitAnswer(view.exam.id, view.currentQuestion.id, answer, timeSeconds)
  }
  ...
  async function handleNext() {
    ...
    if (nextIndex >= view.questions.length) {
      try {
        ...
        setView({ mode: 'results', summary, mastery: m })
      } catch (err) {
      ...
      <div className="h-full overflow-y-auto p-6">
        <div className="max-w-xl mx-auto">
          ...
          <ResultsPanel
            ...
            onTakeAnother={() => setView({ mode: 'idle' })}
          />
        </div>
      </div>
    )
                ...
                <div key={m.id} className="flex items-center gap-3">
                  ...
                  <div className="w-24 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: 'rgba(60,52,137,0.15)' }}>
                    <div
                      ...
                      style={{
                        width: `${Math.round(m.average_score * 100)}%`,
                        backgroundColor: m.mastery_achieved ? '#16a34a' : '#3C3489',
                      }}
                    />
                  </div>
                  ...
                  {m.mastery_achieved === 1 && (
                    <span className="text-xs" style={{ color: '#ca8a04' }}>★</span>
                  )}
                </div>
              ))}
            </div>
              ...
              {history.map(exam => (
                ...
                >
                  <div>
                    ...
                    <span className="ml-2 text-xs" style={{ color: '#6b5fa8' }}>
                      {exam.difficulty_range} · {formatDate(exam.started_at)}
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                    {exam.mastery_achieved === 1 && (
                      <span className="text-xs" style={{ color: '#ca8a04' }}>★</span>
                    )}
                    <span
                      className="font-medium"
                      style={{
                        color: (exam.overall_score ?? 0) >= 0.8 ? '#16a34a'
                          : (exam.overall_score ?? 0) >= 0.6 ? '#ca8a04'
                          : '#dc2626',
                      }}
                    >
